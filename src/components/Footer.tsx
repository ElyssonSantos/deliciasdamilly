import { ChefHat, Phone, Mail, MapPin, Instagram, Clock, Lock } from "lucide-react";
import Watermark from "./Watermark";
import { useAdmin } from "@/contexts/AdminContext";

const Footer = () => {
  const { setIsPasswordModalOpen } = useAdmin();
    ...
    <footer id="contato" className="bg-primary text-primary-foreground py-12">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 mb-8">
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold uppercase tracking-wider text-primary-foreground/90">Informação Legal</h3>
            <Watermark />
          </div>
        </div>

        <div className="border-t border-primary-foreground/20 pt-6 text-center text-sm text-primary-foreground/60 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="flex items-center gap-2">
            <span>© 2026 Delicias da Milly. Todos os direitos reservados.</span>
            {/* Cadeado disfarçado para acesso administrativo */}
            <button
              onClick={() => setIsPasswordModalOpen(true)}
              className="opacity-40 hover:opacity-100 transition-all p-1.5 rounded-full hover:bg-primary-foreground/10 focus:outline-none"
